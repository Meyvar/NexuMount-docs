let pan123 = {
    ...
    init() {
        ...
        $('[action="getCode"]').click(pan123.getCode);
    },
    getCode() {
        let url = location.origin + "/NexuMount-docs/html/views/drivers/pan123/getLogin.html"
        $.ajax({
            url: "https://nexu-mount-api.laoliuhe.qzz.io/?driverType=pan123&action=getLogin&redirectUrl=" + encodeURIComponent(url),
    ...
    exchangeToken(nonce) {
        $.ajax({
            url: "https://nexu-mount-api.laoliuhe.qzz.io/?driverType=pan123&action=exchangeToken&nonce=" + nonce,
            type: "get",
            contentType: "application/json; charset=utf-8",
            success(resp) {
                if (resp.code == 200) {
